...
$checkDir = Join-Path ([System.IO.Path]::GetTempPath()) ('reduce_cd-runtime-' + [guid]::NewGuid())
New-Item -ItemType Directory -Path $checkDir | Out-Null
$checker = Join-Path $PSScriptRoot 'RunReduceCdGameTest.java'
$fixture = Join-Path $checkDir 'packs/reduce_cd'
& python (Join-Path $PSScriptRoot 'prepare_reduce_cd_fixture.py') $fixture
if ($LASTEXITCODE -ne 0) { throw 'Fixture preparation failed' }
Push-Location $checkDir
try {
    & $Java -cp ($jars -join [System.IO.Path]::PathSeparator) $checker --universe (Join-Path $checkDir 'universe') --packs (Join-Path $checkDir 'packs') --tests 'test:reduce_cd' --report (Join-Path $checkDir 'report.xml')
