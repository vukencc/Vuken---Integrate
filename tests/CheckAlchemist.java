...
class CheckAlchemist {
 public static void main(String[] args) throws Exception {
  ...
  try(var stream=Files.walk(root.resolve("data/core/function/class/ability/alchemist"))) {stream.filter(p->p.toString().endsWith(".mcfunction")).sorted().forEach(paths::add);}
  ...
  net.minecraft.world.item.enchantment.Enchantment.DIRECT_CODEC.parse(ops,com.google.gson.JsonParser.parseString(Files.readString(root.resolve("data/core/enchantment/class/alchemist/entity.json")))).getOrThrow();
  net.minecraft.advancements.Advancement.CODEC.parse(ops,com.google.gson.JsonParser.parseString(Files.readString(root.resolve("data/core/advancement/class/alchemist/kill.json")))).getOrThrow();
