...
class RunAlchemist58GameTest {
 ...
 public static void main(String[] args)throws Exception {
  ...
  TestFunctionLoader.registerLoader(new TestFunctionLoader(){
   public void load(BiConsumer<ResourceKey<Consumer<GameTestHelper>>,Consumer<GameTestHelper>> sink){
    sink.accept(ResourceKey.create(Registries.TEST_FUNCTION,Identifier.parse("test:alchemist")),RunAlchemist58GameTest::run);
   }
  });
 ...
 static void run(GameTestHelper helper){
  ...
  try {
   ...
   set(p,"alch.pas1",2);set(p,"pot.sswap",0);
   ...
   for(var pot:pots){System.out.println("POT_BEFORE "+((LivingEntity)pot).getItemBySlot(EquipmentSlot.SADDLE).getComponents());pot.baseTick();pot.baseTick();System.out.println("POT_AFTER "+((LivingEntity)pot).getItemBySlot(EquipmentSlot.SADDLE).getComponents());ok(pot.isPassenger(),"6 potion carrier survives flight initialization");}
   cmd(p,"kill @e[type=splash_potion]");for(var pot:pots)pot.baseTick();
   ok(h.getLevel().getEntities(p,p.getBoundingBox().inflate(64),e->e.entityTags().contains("alch.pool")&&e.isAlive()).size()==1,"6 real potion landing creates exactly one pool");
   System.out.println("ALCHEMIST_58_RUNTIME_PASS checks="+checks);h.succeed();
