...
class RunScout58GameTest {
 ...
 var mob=h.spawn(EntityType.SKELETON,new BlockPos(6,3,2));mob.setNoGravity(true);mob.setTarget(p);fn(d,"5/taunt");for(int i=0;i<20;i++){mob.tickCount++;if(i%10==0)fn(d,"5/taunt");mob.tick();}System.out.println("TAUNT_DEBUG target="+mob.getTarget()+" last="+mob.getLastHurtByMob()+" noAI="+mob.isNoAi()+" canAttack="+mob.canAttack(d));ok(mob.getTarget()==d,"skeleton AI switches aggro from player to decoy");
 ...
 set(p,"scout.pas3",2);cmd(p,"item replace entity @s weapon.mainhand with crossbow[custom_data={range:4.0}]");net.minecraft.advancements.CriteriaTriggers.USING_ITEM.trigger(p,p.getMainHandItem());near(p.getAttributeValue(Attributes.MOVEMENT_SPEED),0.5,"crossbow use also cancels movement slowdown");set(p,"class",2);fn(p,"player_tick");near(p.getAttributeValue(Attributes.MOVEMENT_SPEED),0.1,"changing class removes active compensation");
 ...
 set(p,"range.slc",1);fn(p,"5/cast");var d=(net.minecraft.world.entity.LivingEntity)tagged(p,"scout.decoy").getFirst();ok(score(d,"scout.hits")==5 && d.getHealth()==1024,"level I decoy has five hits");ok(score(p,"range.slc.cd")==0,"decoy has no cooldown while alive");fn(p,"5/cast");ok(tagged(p,"scout.decoy").size()==1,"level I cannot replace live decoy");
 ...
 fn(p,"8/tick");ok(p.getEffect(MobEffects.HASTE).getAmplifier()==1,"level II provides haste II");net.minecraft.advancements.CriteriaTriggers.USING_ITEM.trigger(p,p.getMainHandItem());near(p.getAttributeValue(Attributes.MOVEMENT_SPEED),0.5,"using bow compensates vanilla 0.2 movement multiplier");ok(p.hasEffect(MobEffects.SLOW_FALLING),"using bow at level II gives slow falling");fn(p,"8/tick");fn(p,"8/tick");near(p.getAttributeValue(Attributes.MOVEMENT_SPEED),0.1,"stopping use removes movement compensation");
 extra(p);
