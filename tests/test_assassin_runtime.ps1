...
$fixtureArgs = @($fixture)
if ($BackstabRegression -or $ChainFx) { $fixtureArgs += "--full" }
& $Python (Join-Path $PSScriptRoot 'prepare_assassin_fixture.py') @fixtureArgs
...
try {
    & $Java "-Dassassin.focus=$BackstabFx" "-Dassassin.regression=$BackstabRegression" "-Dassassin.chain=$ChainFx" -cp ($jars -join [System.IO.Path]::PathSeparator) (Join-Path $PSScriptRoot 'RunAssassinGameTest.java') --universe (Join-Path $checkDir 'universe') --packs (Join-Path $checkDir 'packs') --tests 'test:assassin' --report (Join-Path $checkDir 'report.xml')
    ...
    if (Select-String -LiteralPath $runtimeLog -Pattern 'Failed to load function|Serialization errors:|Command execution stopped due to limit|Enchantment run_function effect failed' -Quiet) { throw "Fixture function loading failed. Logs: $checkDir" }
