...
$checkDir = Join-Path ([System.IO.Path]::GetTempPath()) ('skill-fixes-runtime-' + [guid]::NewGuid())
New-Item -ItemType Directory -Path $checkDir | Out-Null
$checker = Join-Path $PSScriptRoot 'RunSkillFixesGameTest.java'
$fixture = Join-Path $checkDir 'packs/alchemist'
& python (Join-Path $PSScriptRoot 'prepare_skill_fixes_fixture.py') $fixture
if ($LASTEXITCODE -ne 0) { throw 'Fixture preparation failed' }
Push-Location $checkDir
try {
    & $Java -cp ($jars -join [System.IO.Path]::PathSeparator) $checker --universe (Join-Path $checkDir 'universe') --packs (Join-Path $checkDir 'packs') --tests 'test:skill_fixes' --report (Join-Path $checkDir 'report.xml')
