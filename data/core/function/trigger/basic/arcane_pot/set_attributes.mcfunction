...
execute positioned ~ ~-1.6 ~ positioned ^ ^ ^-0.7 store result entity @n[type=armor_stand,tag=pot_as] data.id int 1 run scoreboard players get @p marker
...
execute positioned ~ ~-1.6 ~ positioned ^ ^ ^-0.7 as @p if predicate operation:stats/sneaking positioned ~ ~1.6 ~ positioned ^ ^ ^0.7 run tag @n[type=armor_stand,tag=pot_as] add from_sneaking