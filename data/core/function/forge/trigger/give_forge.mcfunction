...
execute unless items entity @s weapon.mainhand #armor unless items entity @s weapon.mainhand *[enchantments~[{enchantments:"#weapon_id"}]] unless items entity @s weapon.mainhand *[custom_data~{Offhand:1b}] run return fail
...
execute as @n[tag=Forge] if entity @s[tag=rotate] as @n[type=item_display,tag=forge_show] at @s run rotate @s ~180 ~
